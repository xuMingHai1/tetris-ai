...
`RecoveryRobustnessBenchmark` is the next diagnostic layer and still does not define policy. For one post-placement board it first applies the known preview piece using production SURVIVAL rank 1, then evaluates every tetromino type as the following unknown piece through production action-native reachability. The calibration compares known clean-witness states with the final five selected states before game-over from failed `preserve-baseline` guard games. It records immediate preview recovery, recovery board health, unplayable unknown piece types, min/average/max unknown-piece reachable outcomes, and worst post-unknown SURVIVAL board facts. Its purpose is feature separation: only if these facts distinguish successful construction from cumulative failure should a later PR turn them into a candidate safety policy. The first calibration found a strong observed separation in vertical reserve: all 22 clean-witness samples had recovery headroom 8–17 and worst-case post-unknown headroom 8–16, while all 35 failed-tail samples were at most 2 on both measures (or unrecoverable). The separation was already present four successful placements before game-over. By contrast, minimum unknown-piece reachable outcomes overlapped at 9 and recovery-hole counts overlapped between the classes, so branching and holes alone remain weak safety proxies. This evidence is promising but not yet a runtime threshold: the next calibration must add longer failure lead-time and healthy long-lived runtime controls to measure false positives. `RecoveryReserveLeadTimeApplication` performs that next calibration without changing the probe or runtime planner. It extends each failed preserve-baseline game to the final 30 successful placements and samples only current BUILD_SHAPE games that reach the complete piece limit, using a deterministic every-five-decisions cadence plus the final 30 decisions. Candidate recovery-headroom and post-unknown-headroom cutoffs 2/4/6/8 are swept only to measure clean/control warnings, healthy-control false positives by sample and game, failed-game detection coverage, and earliest observed warning lead-time. The sweep is evidence generation, not threshold selection. In the first 10×250 run, recovery-headroom cutoff 2 produced no clean-witness warnings, 4/740 sampled healthy-control warnings (all four consecutive states in seed 1003), and detected all 7 failed preserve-baseline games with 6–29 placements of observed lead-time (16.3 average). Higher cutoffs increased healthy false positives sharply. The four healthy low-reserve states still had only 10–11 recovery holes, whereas recoverable failed states with the same low-reserve condition had 15–60 holes. That conditional separation is a candidate feature interaction for independent validation, not permission to encode a holes threshold. `RecoveryReserveValidationApplication` performs that independent check without another parameter sweep. It freezes the calibrated interaction: an unrecoverable preview is a warning; otherwise both recovery headroom <= 2 and recovery holes >= 15 are required. On seeds 2000–2019, known clean witnesses produced 0/22 warnings; 15/20 current BUILD_SHAPE games reached 1000 pieces and contributed 3360 sampled healthy controls with zero warnings; all 16 failed preserve-baseline games were detected with 5–29 placements of lead-time (18.813 average). This confirms out-of-sample class separation but does not yet show that the signal predicts failures of the current production planner, because 5/20 current BUILD_SHAPE games themselves failed before the 1000-piece horizon and were excluded from the healthy-control population by protocol. `RecoveryReserveRuntimeValidationApplication` then ran the production `BuildShapeActionPlanningAgent` once per fresh seed 3000–3039 with a 1000-piece horizon. The frozen warning rule was reused unchanged. The 40-game result contained 28 healthy and 12 failed games; 6272 sampled healthy states produced 7 warnings (0.1116%) across 4 healthy games, while all 12 production failures were detected. First-warning lead-time ranged from 0 to 29 placements with a 19.0 average; 10/12 failures warned at least 12 placements early, while 2/12 warned only on the final successful placement. A healthy game at seed 3007 warned at decisions 100 and 105 and still survived to 1000, demonstrating that a single warning can be transient. Seed 3004 warned at 998–1000 and is horizon-censored rather than proven healthy after the warning. `RecoveryReservePersistenceValidationApplication` then kept the same frozen headroom/holes signal and evaluated every successful placement on fresh seeds 4000–4029. The 30-game result contained 16 healthy and 14 failed games. Three healthy games produced 18 warning episodes and all 18 recovered; healthy episode length averaged 3.944 and reached 15. All 14 failed games were detected, but 41 of their 55 warning episodes also recovered before the final terminal episode. Failed recovered episodes reached length 18, while terminal episodes ranged from only 1 to 22 placements; five terminal episodes were length 1. First warning occurred 0–662 placements before failure (91.643 average). Episode duration therefore does not form a usable separator: a simple consecutive-warning threshold would create both false positives and false negatives. `RecoveryInterventionComparisonApplication` then moved from passive prediction to a benchmark-only paired action experiment on fresh seeds 5000–5029. The 30×1000 baseline and intervention arms tied on all 30 seeds; both reached the horizon in 16/30 games, with identical 12.133 average best HEART visual error and no clean completions. More importantly, all 274 frozen-warning decisions already had production BUILD_SHAPE selectedRank=1 and objectiveApplied=false. The proposed intervention therefore replaced 0/274 plans, cleared 0/274 warnings and left all 274 warnings present after re-requesting the same deterministic SURVIVAL top-1 action. The current-head raw recovery probe averaged about 12.373 ms and reached about 29.777 ms. This falsifies a warning-triggered fallback-to-SURVIVAL policy: the warning identifies states where one-step SURVIVAL is already active yet still leaves weak recovery reserve. `RecoveryCandidateScanApplication` then validated recovery-action availability on fresh seeds 6000–6019 with a 20×1000 production BUILD_SHAPE run. All 137 frozen-warning states already had selectedRank=1. Forty-eight of 137 states (35.0%) had at least one alternative action that cleared the frozen warning. When an alternative existed, earliest clearing rank averaged 3.458: rank 2 accounted for 29 states, rank 3 for 6, and rank 4 for 5, so 40/48 clearing states had an answer in the existing SURVIVAL top 4. All 48 earliest-clearing alternatives had zero immediate HEART visual-error delta. Best recovery-headroom gain averaged +0.796 and reached +3. Six preview-unrecoverable states had no clearing alternative and all came from failed games. The six failed games had no clearing alternative at their final successful placement, although five still had one 3–16 placements earlier. Full scans averaged about 45.793 ms and reached about 143.043 ms, so the exhaustive scanner remains evidence rather than a runtime implementation. `RecoveryClearingPlannerComparisonApplication` then executed that first-clearing policy on fresh seeds 7000–7029 in a 30×1000 paired benchmark. Production reached the horizon in 14/30 games and the recovery arm in 16/30. Twelve seeds improved, five regressed and thirteen tied, with total piece delta +625, but the tails were large: +424 at best and -614 at worst. Four baseline failures became horizon survivors, while two baseline survivors became failures. The recovery arm observed 200 warned rank-1 decisions and replaced 58 of them; 39 replacements were rank 2, yet the worst regression (seed 7026, 1000→386) used rank 2 for six of seven replacements, so a simple rank cap is not justified. All 58 replacements had zero immediate HEART visual-error delta, indicating that the mixed outcome comes from downstream trajectory effects rather than immediate construction sacrifice. Average decision latency rose from about 1.733 ms to 14.380 ms. `RecoveryOneShotCounterfactualApplication` then isolated one action at a time on fresh seeds 8000–8019. It produced 52 qualifying states: 22 improved, 17 regressed and 13 tied. Baseline and recovery branches reached the fixed 50-step horizon in 27 and 26 samples respectively, with 14 horizon wins and 15 losses; mean survived-depth delta was +3.019 but the median was 0. Every clearing action had zero immediate HEART visual-error delta. The decisive evaluation-only split was trajectory outcome: samples from baseline games that eventually reached 1000 pieces were 0 improved / 11 regressed / 11 tied with mean depth delta -13.091, while samples from games that later failed were 22 / 6 / 2 with mean +14.833. Rank 2 remained mixed, so clearing rank is not an eligibility boundary. Eventual game outcome is future information and cannot be used at runtime. `RecoveryEligibilityFeatureAuditApplication` therefore performs calibration-only feature discrimination on fresh seeds 9000–9029. It reuses the same 50-step one-shot label but records only pre-outcome BUILD_SHAPE, placement and recovery-probe facts as candidate policy inputs. Numeric features are compared with threshold-free ROC AUC and direction only; no cutoff is selected from this population. Baseline eventual game outcome is retained solely as explicitly marked evaluation metadata. Any candidate discriminator must be frozen and independently validated before another runtime-action experiment.
