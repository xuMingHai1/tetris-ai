...
Because the target is fixed while the settled board persists across pieces, the board still carries multi-turn progress without a second mutable objective-state store. Runtime BUILD_SHAPE remains the greedy one-turn planner. A benchmark-only preview-lookahead variant now tests one bounded two-ply hypothesis: each safety-eligible current top-five candidate may be projected through the known preview piece, whose own action-native top five is re-ranked and independently guarded by its future SURVIVAL baseline, Risk Controller and Safety Budget. The experiment does not widen either turn's top-five survival-quality prior and does not change runtime objective selection until benchmark evidence justifies adoption.
...
`build-shape` emits Tetris-aware target telemetry: full reachable-candidate count, actual creative shortlist count, safety-eligible count, selected survival rank, required matches, forbidden/support occupancy, visual-error delta, required-cell coverage, exact clean completion, danger suppression, risk profile and safety filtering. This keeps reachability breadth, survival-quality prior and risk eligibility separately measurable. `build-shape-preview` emits the same telemetry with a distinct strategy label. `compare-shape` runs the deterministic SURVIVAL baseline and greedy BUILD_SHAPE on identical seeds, while `compare-shape-preview` directly compares greedy BUILD_SHAPE with the preview-lookahead hypothesis on identical seeds.
...
- High-level objectives may change preference among deterministic reachable plans but must not replace movement legality, collision or row clearing. Heuristic rank is never proof of safety; non-baseline choices still require an explicit safety boundary relative to SURVIVAL top-1. BUILD_SHAPE additionally retains heuristic top-5 as an evidence-backed survival-quality prior because the full-reachable experiment regressed long-term survival without improving clean shape completion.
