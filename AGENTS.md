...
- `BUILD_SHAPE` 等 creative objective 必须基于生产 `resultingBoard` 的 post-row-clear 事实评分，不能维护第二套棋盘；当前 board state 只有 occupancy 时不得推断或伪造 settled-piece color，颜色目标要等显式 color-aware state model。Creative target 必须显式区分视觉 REQUIRED、视觉 FORBIDDEN 与允许的物理 SUPPORT，不能把“目标格都被占用”误当成完整图形；clean completion 还必须保证 FORBIDDEN 为空。BUILD_SHAPE 保留 heuristic top-5 作为当前 runtime 的 survival-quality prior，同时记录全量 action-native reachable 数量用于观测；SURVIVAL top-1 始终是风险 baseline，任何非 baseline 候选都必须通过相对 SURVIVAL top-1 的 `ObjectiveSafetyBudget`。不要直接扩大 runtime 到 full reachable envelope；AI Benchmark #30 已证明 top-5 是 known clean construction path 的真实 blocker，#48 又证明这些 witness state 具有显著 continuation viability，因此允许在独立 benchmark agent 中做 full-reachable + explicit guard 对照，但 runtime adoption 仍必须同时满足 construction improvement 与 survival regression gate。一步 preview lookahead 已通过 20×500 benchmark 验证但未通过采用门槛；不要继续盲目增加 lookahead 深度。AI Benchmark #29 已找到两个真实 clean HEART witness（seed 1001/12 pieces、1008/10 pieces），因此 HEART 可构造性不再是当前问题。AI Benchmark #30 进一步确认这两个 witness 的 22 步中有 19 步在 top-5 外、17 步进入 DANGER、22 步全部被 raw Safety Budget 拒绝，因此不能只放宽单一阈值。AI Benchmark #31 的 target-aware hole audit 又确认：虽然 FORBIDDEN / SUPPORT_ALLOWED 占了绝大多数 raw hole observations，但只排除 FORBIDDEN 后仍 0/22 通过，说明 hole 不是唯一 blocker。construction safety envelope 已记录 clean witness 的绝对 headroom、height、hole composition、bumpiness、survival rank、shape progress，以及当前/下一 piece reachable outcomes。viability calibration 已在相同 deterministic future piece suffix 下确认：22/22 known clean witness state 走满 depth-4 bounded continuation，20/22 被生产 SURVIVAL top-1 接管后继续完整 24 块，其余仍继续 21/20 块，21/22 不弱于同一步 SURVIVAL baseline。该证据允许 benchmark-only `ConstructionSafetyGuard` 实验放开 full reachable current candidates，但不能直接改变 runtime。第一轮 guard 已只使用已知 preview piece 的真实 action-native reachable outcome capacity，并比较 preserve-baseline / retain-half / any-continuation 三档。10×250 结果全部未通过 survival gate：runtime BUILD_SHAPE 10/10 到达 250，三档分别只有 3/10、2/10、2/10；虽然最佳 visual error 从 10 改善到 4~6，但 clean completion 仍为 0。因此 one-piece reachable-count guard 已被否决为 runtime safety model；不要通过继续微调 50%/100% 比例来挽救该设计。下一阶段 `RecoveryRobustnessBenchmark` 必须先做区分能力验证：正样本使用 known clean witness，负样本使用失败 `preserve-baseline` guard 局在 game-over 前最后 5 个 selected state；先让已知 preview 走 SURVIVAL top-1 recovery，再对 I/J/L/O/S/T/Z 七种未知下一块扫描 action-native reachability 与 recovery 后 board health。首轮 calibration 已观察到：22 个 clean witness 的 recovery headroom / worst-case post-unknown headroom 最低均为 8，而 35 个 failed-tail 样本最高均为 2（或不可恢复），并且距离 game-over 4 个成功 placement 时仍保持这一分离；但 min unknown reachable 在 9 有重叠，recovery holes 也有明显重叠。这个结果只证明 recovery 后的垂直余量值得继续研究，不是 `8` 或 `2` 的 runtime 阈值。下一阶段 `RecoveryReserveLeadTimeApplication` 正是该 calibration：失败 guard 最多回看 game-over 前 30 个成功 placement；healthy control 只能来自当前 runtime BUILD_SHAPE 确实到达完整 piece limit 的局，并按每 5 个 decision + 最后 30 个 decision 采样。cutoff 2/4/6/8 只用于同时报告 clean/control false positive 与失败预警 lead-time，不得因为某个 cutoff 在 10 个 seed 上表现好就直接写进 runtime。首轮 10×250 lead-time calibration 已得到：`recovery-headroom<=2` 对 clean witness 0 warning、healthy control 4/740 sample warning（只出现在 seed 1003 连续 4 步），7/7 失败局均被捕捉，observed lead-time 最少 6、平均 16.3、最多 29 步；cutoff 4/6/8 的 healthy false positive 明显变差。低-reserve 条件下 healthy control 的 recovery holes 为 10~11，而失败状态为 15~60，但这仍是同一 seed 集上的条件分布，不得直接写成 `holes>=15` 或任何 composite runtime guard。`RecoveryReserveValidationApplication` 已完成 out-of-sample 验证：固定独立 seed 2000–2019，healthy runtime 1000 pieces，失败 preserve-baseline horizon 500 pieces，并冻结组合信号为“preview 不可恢复直接 warning；否则 `recoveryHeadroom<=2 && recoveryHoles>=15`”。最终结果为 known clean 0/22 warning；15/20 当前 runtime 局达到 1000 pieces，其中 sampled healthy controls 0/3360 warning；preserve-baseline 16 个失败局 16/16 被检测，observed lead-time 最少 5、平均 18.813、最多 29。5/20 当前 runtime 局本身未达到 1000 pieces，因此该结果还没有证明 signal 能预测生产 BUILD_SHAPE 自己的失败。`RecoveryReserveRuntimeValidationApplication` 已完成生产失败验证：全新 seed 3000–3039、40×1000，只运行生产 `BuildShapeActionPlanningAgent`，frozen signal 不变。结果为 28/40 healthy、12/40 failed；healthy sampled states 6272 中 7 次 warning（0.1116%），分布于 4/28 healthy games；12/12 生产失败全部被检测，首次 warning lead-time 最少 0、平均 19、最多 29，其中 10/12 至少提前 12 个 placement，2/12 只在最后一个成功 placement 才首次 warning。healthy seed 3007 在 decision 100/105 warning 后仍存活到 1000，证明单次 warning 可以恢复；seed 3004 的 998–1000 warning 则受测试 horizon censoring。下一阶段 `RecoveryReservePersistenceValidationApplication` 已完成时间结构验证：全新 seed 4000–4029、30×1000、逐 successful placement probe，frozen signal 不变。结果为 16 healthy / 14 failed；healthy 3/16 games 出现 warning，共 18 个 episode 且 18/18 最终恢复，平均长度 3.944、最长 15；14/14 failed 全部被检测，55 个 failed episode 中 41 个也曾恢复，14 个失败局最终各有一个 terminal episode，terminal episode 最短只有 1、最长 22；failed recovered episode 最长 18，首次 warning lead-time 最少 0、平均 91.643、最多 662。该结果否决了“连续 N 次 warning 就介入”的简单 persistence 阈值：healthy 能从 15-step warning 恢复，failed 也能从 18-step warning 恢复，同时 5/14 最终 terminal episode 只有 1 step。`RecoveryInterventionComparisonApplication` 已完成 paired action experiment：全新 seed 5000–5029、30×1000，baseline 与 intervention 30/30 完全 tied，二者均 16/30 到达 1000 pieces，total/average piece delta 都为 0，平均 best HEART visual error 都为 12.133，clean completion 都为 0。关键原因是 274/274 frozen-warning decisions 的 production BUILD_SHAPE 已经 selectedRank=1、objectiveApplied=0，也就是现有 deterministic SURVIVAL top-1；因此 intervention 实际 plan replacement 为 0/274，SURVIVAL 后 warning 清除为 0/274。warning 原因为 260 次 low-reserve+structural-debt、14 次 preview-unrecoverable。当前 head raw recovery probe 平均约 12.373 ms、最大约 29.777 ms。该结果否决了“warning -> fallback SURVIVAL top-1”runtime guard：它只增加 probe 成本而不改变动作。`RecoveryCandidateScanApplication` 已完成 recovery action availability 验证：全新 seed 6000–6019、20×1000 的 production BUILD_SHAPE 轨迹中共观察到 137 个 frozen-warning states，137/137 都是 selectedRank=1，non-top1 warning 为 0。137 个 states 中 48 个（35.0%）存在至少一个能清除 frozen warning 的其它 action-native reachable candidate；healthy warning states 为 31，其中 21 个有 clearing alternative；failed-game warning states 为 106，其中 27 个有 alternative。平均 reachable candidates 19.255，存在 alternative 时最早 clearing rank 平均 3.458；rank2 有 29 个、rank3 6 个、rank4 5 个，因此 40/48 在原 SURVIVAL top4 内。所有 48 个 earliest-clearing alternatives 的 HEART visual-error delta 都为 0。best recovery-headroom gain 平均 +0.796、最大 +3。131 个 low-reserve+structural-debt states 中 48 个可清除；6 个 preview-unrecoverable states 中 0 个可清除且全部来自最终失败局。fresh population 有 6 个失败局，最后一个成功 placement 的 warning state 6/6 都已无 clearing alternative，但其中 5 局在死亡前 3–16 placements 仍曾存在 alternative。全量 candidate scan 平均约 45.793 ms、最大约 143.043 ms，只适合作为 evidence，不可直接当 runtime 实现。下一阶段 `RecoveryClearingPlannerComparisonApplication` 已完成全新 seed 7000–7029、30×1000 paired causal validation：production baseline 有 14/30 到达 1000，first-clearing recovery arm 有 16/30；12 个 seed 改善、5 个回归、13 个持平，总 piece delta +625、平均 +20.833，但最大改善 +424、最大回归 -614。4 个 baseline failure 被救到 1000，同时 2 个 baseline survivor 被 recovery 打死，因此不能按 aggregate 净收益直接生产化。recovery arm 共评估 21624 个 rank1 decisions，200 次 warning，58 次实际 replacement、142 次 unresolved；平均 replacement rank 3.138，39/58 为 rank2，但最严重回归 seed 7026 从 1000 降到 386，其 7 次 replacement 中 6 次就是 rank2，说明简单 rank cap 不能解决问题。58/58 replacement 的 immediate HEART visual-error delta 都为 0，回归来自后续轨迹而非当步构造损失。平均 production decision latency 1.733 ms，recovery arm 14.380 ms，平均增加 12.647 ms/decision；平均 measured recovery overhead 13.739 ms、最大 101.111 ms。该结果否决直接 production integration，也不得从同一 seeds 继续调 rank cutoff。`RecoveryOneShotCounterfactualApplication` 已完成全新 seed 8000–8019、20×1000、固定 50-step continuation 的状态级单次动作因果验证。共得到 52 个 rank1-warning + first-clearing qualifying states：22 improved、17 regressed、13 tied；baseline/recovery 到达 50-step horizon 分别 27/26，horizon wins 14、losses 15；平均 survived-depth delta +3.019，但中位数为 0，且 clearing 当步 HEART visual-error delta 52/52 都为 0，因此 aggregate 平均值不能作为生产化依据。最关键的事后分层是：来自最终能到 1000 pieces 的 baseline games 的 22 个 samples 中 0 improved、11 regressed、11 tied，平均 depth delta -13.091；来自最终失败 baseline games 的 30 个 samples 中 22 improved、6 regressed、2 tied，平均 +14.833。rank2 仍混合（14 improved / 9 regressed / 11 tied），所以 rank cap 不是 eligibility rule。baseline 最终是否失败只可作为 evaluation label，runtime 不可观测。下一阶段 `RecoveryEligibilityFeatureAuditApplication` 必须只审计 intervention 前即可获得的事实，使用 fresh seed 9000–9029、30×1000，并延续相同 50-step one-shot label；numeric features 只报告 threshold-free ROC AUC 和方向，不从 calibration population 选择 cutoff；eventual baseline game result 必须显式标为 evaluation-only，不能进入 numeric/categorical policy feature。若发现候选 discriminator，后续必须冻结后再做新的 out-of-sample validation，仍不得直接接入 runtime。任何新的 BUILD_SHAPE safety policy 都必须和现有 SURVIVAL/TUCK_HUNTER 的通用 `ObjectiveSafetyBudget` 分开评估，避免用 creative semantics 污染通用 survival boundary；不得从 feasibility beam search 直接复制一个 runtime planner。benchmark-only feasibility search 可以遍历 full action-native reachable outcomes，因为它回答可达性问题；bounded miss 仍不得宣称 target 不可能。目标机会不存在或超出 budget 时 runtime 必须回退到 SURVIVAL。`ObjectiveRiskController` 只能根据 SURVIVAL top-1 的结果棋盘选择风险档位，不能根据待评估 objective candidate 反向改变 budget。runtime 可在 STRICT / CONSERVATIVE / BALANCED 间切换，但必须保持 `additional holes == 0`；RISKY 仍只用于 benchmark calibration。BUILD_SHAPE 在 DANGER 下必须停止 creative deviation并返回 SURVIVAL top-1。
...
`.github/workflows/ai-benchmark.yml` 同时作为手动 `workflow_dispatch` 和 reusable workflow，实际 benchmark 命令、参数校验与 artifact 收集只能维护这一份。`.github/workflows/ai-benchmark-pr.yml` 负责同仓库 PR 的自动选择：recovery eligibility feature audit 改动优先 `recovery-eligibility-feature-audit (30 × 1000, seed 9000, continuation 50)`，one-shot recovery counterfactual 改动使用 `recovery-one-shot-counterfactual (20 × 1000, seed 8000, continuation 50)`，paired first-clearing recovery planner 改动使用 `recovery-clearing-planner-comparison (30 × 1000, seed 7000)`，warned-state recovery candidate scan 改动使用 `recovery-candidate-scan (20 × 1000, seed 6000)`，paired recovery intervention 实验改动使用 `recovery-intervention-comparison (30 × 1000, seed 5000)`，recovery warning persistence 验证改动使用 `recovery-reserve-persistence-validation (30 × 1000, seed 4000, every successful placement)`，生产 BUILD_SHAPE recovery failure 验证改动使用 `recovery-reserve-runtime-validation (40 × 1000, seed 3000)`，out-of-sample recovery-reserve 验证改动使用 `recovery-reserve-validation (20 × healthy 1000 / failure 500, seed 2000)`，recovery-reserve lead-time 校准改动使用 `recovery-reserve-lead-time (10 × 250)`，recovery-robustness 校准改动使用 `recovery-robustness (10 × 250)`，construction-guard 实验改动使用 `compare-shape-guard (10 × 250)`，其它 BUILD_SHAPE / shape 改动使用 `shape-feasibility (20 seeds, depth 24, beam 128)`，objective risk/safety 改动使用 `compare-adaptive (10 × 250)`，action-native reachability / shared board facts 改动使用 `action-provenance (10 × 250)`；一个 PR revision 最多选择一个 deterministic benchmark，优先级为 recovery-eligibility-feature-audit > recovery-one-shot-counterfactual > recovery-clearing-planner-comparison > recovery-candidate-scan > recovery-intervention-comparison > recovery-reserve-persistence-validation > recovery-reserve-runtime-validation > recovery-reserve-validation > recovery-reserve > recovery > guard > shape > objective > action，新的 push 必须取消旧 run。自动 PR benchmark 只允许目标为当前 default branch 的 same-repository PR，外部 fork 不得占用 self-hosted runner；自动路径不得调用 Jev 或读取 provider secret。手动 workflow 继续保留完整模式；`shape-feasibility` 使用独立的 `feasibility_depth` 与 `feasibility_beam_width`，不要把普通 gameplay 的 `max_pieces=500` 直接当成 feasibility depth。BUILD_SHAPE artifact 应保留逐 decision、逐 game construction diagnostics，以及 feasibility result/witness 和 clean-witness runtime constraint audit（若存在）；constraint audit 还应保留 target-aware hole breakdown 与明确标注为 counterfactual 的 forbidden-hole-exclusion 字段；clean witness 还应输出 `construction-safety-envelope.csv` 与 `construction-viability.csv`；recovery robustness 校准输出 `recovery-robustness.csv`，lead-time calibration 另输出 `recovery-reserve.csv`，out-of-sample validation 输出 `recovery-reserve-validation.csv`，production runtime failure validation 输出 `recovery-reserve-runtime-validation.csv`；warning persistence validation 分别输出 warning、episode 与 per-game CSV；paired recovery intervention 输出 per-warning decision 与 paired per-game CSV；recovery candidate scan 输出 per-candidate、per-warning-state 与 per-game CSV；paired first-clearing recovery planner 输出 per-warning decision 与 paired per-game CSV；one-shot recovery counterfactual 输出 per-state 与 per-game CSV；recovery eligibility feature audit 另外输出 pre-outcome feature samples、numeric AUC audit、categorical audit 与 per-game CSV。这些 min/max、rollout/search horizon 与 class-separation facts 都只代表 observed evidence。PR 自动 shape-feasibility 固定 viability search depth 4 / beam 32 / greedy depth 24，以限制 self-hosted runner 成本；更深校准使用手动 workflow。Jev benchmark（含手动 `compare` 中的 Jev 部分）只有显式确认外部调用成本并存在 `TYPESAFE_API_KEY` repository secret 时才能运行，且单次最多 200 个潜在 Jev decision。benchmark 结果应上传 artifact，不提交生成结果到源码仓库。
