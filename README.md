...
该模式若找到 clean completion，会输出逐 piece 的 `AiPlan` witness，并立即运行 runtime constraint audit 与 construction safety envelope；未找到只表示当前 depth / beam 预算没有找到构造路径。constraint audit 写入 `shape-witness-audit.csv`；成功路径的绝对 board-health 与 continuation facts 写入 `construction-safety-envelope.csv`。envelope 的 extrema 是 observed evidence，不应直接复制成新的安全阈值。
